...
async def test_sync_cover_art_hash_map_prevents_variant_pingpong(hass: HomeAssistant, tmp_path: Path) -> None:
    ...
    for clip in clips:
        ...
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(b"fLaC" + b"\x00" * 50)
        clips_state[clip.id] = ManifestEntry(path=rel_path)
...
async def test_async_reconcile_retags_when_gpt_description_prompt_changes(
    hass: HomeAssistant,
    tmp_path: Path,
) -> None:
    ...
    old_hash = clip_meta_hash(clip)
    clip.gpt_description_prompt = "desc B"
    ...
    assert audio.rendered == []
    assert len(audio.retag_calls) == 1
    _target, meta = audio.retag_calls[0]
    assert meta.comment == "desc B"
    assert meta.suno_style_summary == "desc B"
    new_hash = clip_meta_hash(clip)
    assert new_hash != old_hash
    assert library.state.clips[clip.id].meta_hash == new_hash
    assert library.errors == 0
...
async def test_retag_aborts_when_image_url_set_but_no_bytes_available(hass: HomeAssistant, tmp_path: Path) -> None:
    ...
    assert not library.state.clips[clip.id].embedded_art_hash, "sentinel must not be set on failure"
...
async def test_video_convert_failure_records_failed_marker(hass: HomeAssistant, tmp_path: Path) -> None:
    ...
    assert library.state.clips[clip.id].video_art_settings is None
...
async def test_video_convert_success_clears_failed_marker(hass: HomeAssistant, tmp_path: Path) -> None:
    ...
    with patch(
        ...
        await library.async_reconcile(_options(sync_dir), SunoData(liked_clips=[clip]))
        assert library.state.clips[clip.id].video_art_failed is not None

        await library.async_reconcile(_options(sync_dir), SunoData(liked_clips=[clip]))

    entry = library.state.clips[clip.id]
    assert entry.video_art_failed is None
