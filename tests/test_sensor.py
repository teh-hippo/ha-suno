...
async def test_sensor_setup_creates_all_sensors(hass: HomeAssistant, mock_suno_client: AsyncMock) -> None:
    ...
    # credits, liked_songs, cache_size (library sync disabled by default - no path)
    assert len(states) == 3
...
# ── Library sync sensor unit tests ─────────────────────────────────
...
def test_sync_status_idle_when_no_dm() -> None:
    ...
    sensor = _make_sync_sensor(SunoDownloadStatusSensor, dm_mock=None)
    assert sensor.native_value == "idle"


def test_sync_status_syncing_when_running() -> None:
    """Returns 'syncing' when download manager is running."""
    dm = MagicMock()
    dm.is_running = True
    dm.errors = 0
    sensor = _make_sync_sensor(SunoDownloadStatusSensor, dm_mock=dm)
...
def test_library_files_returns_total() -> None:
    """Returns total_files count from download manager."""
    dm = MagicMock()
    dm.total_files = 42
    value_fn = _SYNC_SENSORS[0][2]  # library_files
    sensor = _make_sync_sensor(_SimpleSensor, dm_mock=dm, value_fn=value_fn)
    assert sensor.native_value == 42


def test_library_files_zero_when_no_dm() -> None:
    """Returns 0 when download_manager is None."""
    value_fn = _SYNC_SENSORS[0][2]
    sensor = _make_sync_sensor(_SimpleSensor, dm_mock=None, value_fn=value_fn)
...
def test_sync_remaining_zero_when_no_dm() -> None:
    """Returns 0 when download_manager is None."""
...
def test_library_size_returns_mb() -> None:
    """Returns library_size_mb from download manager."""
    dm = MagicMock()
    dm.library_size_mb = 123.4
    value_fn = _SYNC_SENSORS[2][2]  # library_size
    sensor = _make_sync_sensor(_SimpleSensor, dm_mock=dm, value_fn=value_fn)
    assert sensor.native_value == 123.4


def test_library_size_zero_when_no_dm() -> None:
...
# ── Library sync last_run and result sensors ───────────────────────


def test_last_sync_returns_datetime() -> None:
    """Returns parsed datetime from download_manager.last_download."""
    from datetime import datetime

    dm = MagicMock()
    dm.last_download = "2026-03-22T08:00:00+00:00"
    value_fn = _SYNC_SENSORS[3][2]  # last_sync
    sensor = _make_sync_sensor(_SimpleSensor, dm_mock=dm, value_fn=value_fn)
...
def test_last_sync_none_when_no_dm() -> None:
...
def test_last_sync_result_returns_summary() -> None:
    """Returns last_result string from download manager."""
    dm = MagicMock()
    dm.last_result = "3 new songs, 1 removal"
    value_fn = _SYNC_SENSORS[4][2]  # last_sync_result
    sensor = _make_sync_sensor(_SimpleSensor, dm_mock=dm, value_fn=value_fn)
    assert sensor.native_value == "3 new songs, 1 removal"


def test_last_sync_result_empty_when_no_dm() -> None:
...
def test_library_files_breakdown_returns_source_counts() -> None:
...
def test_last_sync_has_no_state_class() -> None:
    """last_sync should NOT have state_class=MEASUREMENT."""
    cfg = _SYNC_SENSORS[3]  # last_sync
...
def test_last_sync_result_has_no_state_class() -> None:
    """last_sync_result should NOT have state_class=MEASUREMENT."""
    cfg = _SYNC_SENSORS[4]  # last_sync_result
...
def test_sync_status_attributes_no_errors_key_when_no_dm() -> None:
    """sync_status omits errors key when download_manager is None."""
    sensor = _make_sync_sensor(SunoDownloadStatusSensor, dm_mock=None)
    attrs = sensor.extra_state_attributes
